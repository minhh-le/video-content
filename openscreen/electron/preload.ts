...
contextBridge.exposeInMainWorld("electronAPI", {
	...
	// Main forwards automation commands here; the renderer command bus replies
	// via sendAutomationResult. Only exercised when the app runs with
	// OPENSCREEN_AUTOMATION=1, but the channels are always wired so any window
	// can participate.
	onAutomationCommand: (
		callback: (payload: { id: string; command: string; args: Record<string, unknown> }) => void,
	) => {
		const listener = (
			_event: unknown,
			payload: { id: string; command: string; args: Record<string, unknown> },
		) => callback(payload);
		ipcRenderer.on("automation:command", listener);
		return () => ipcRenderer.removeListener("automation:command", listener);
	},
	sendAutomationResult: (payload: {
		id: string;
		ok?: boolean;
		result?: unknown;
		error?: string;
		...
		ipcRenderer.send("automation:result", payload);
	},
});
