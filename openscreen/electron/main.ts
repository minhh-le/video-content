...
import { startAutomationBridge } from "./automation/bridge";
import {
...
app.whenReady().then(async () => {
	...
	// Optional local automation bridge for the MCP server. No-op unless the app
	// was launched with OPENSCREEN_AUTOMATION=1.
	try {
		startAutomationBridge();
	} catch (error) {
		console.error("Failed to start automation bridge:", error);
	}
