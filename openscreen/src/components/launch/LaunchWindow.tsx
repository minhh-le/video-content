...
import { useAutomationActions } from "@/automation/useAutomationActions";
...
export function LaunchWindow() {
	...
	// Recording-control surface for the automation bus (MCP). Registered here
	// because useScreenRecorder lives in this window.
	useAutomationActions({
		recording_status: () => ({ recording, paused, elapsedSeconds }),
		start_recording: () => {
			if (recording) {
		...
		set_capture_options: (args) => {
			if (recording) {
				throw new Error("Cannot change capture options while recording.");
			}
			if (typeof args.microphone === "boolean") {
				setMicrophoneEnabled(args.microphone);
			}
			if (typeof args.systemAudio === "boolean") {
				setSystemAudioEnabled(args.systemAudio);
			}
			if (typeof args.webcam === "boolean") {
				setWebcamEnabled(args.webcam);
			}
			return { ok: true };
		},
	});
